...
class Cropper extends Layer
	constructor: (options) ->
		...
		@updateCrop = =>
			minX = @cropHandleNW.x + @cropHandleNW.width / 2
			minY = @cropHandleNW.y + @cropHandleNW.height / 2
			maxX = @cropHandleSE.x + @cropHandleNW.width / 2
			maxY = @cropHandleSE.y + @cropHandleNW.height / 2
			@crop.x =  minX
			@crop.y =  minY
		...
		for layer in [@cropHandleNW, @cropHandleNE, @cropHandleSE, @cropHandleSW]
			...
			layer.draggable.constraints =
				x: -handleConstraint,
				y: -handleConstraint,
				width: @.width + handleConstraint*2,
				height: @.height + handleConstraint*2
...
destroyCropper = () ->
	for layer in cameraUI.children
		if layer.props.name is cropper.name
			layer.destroy()
